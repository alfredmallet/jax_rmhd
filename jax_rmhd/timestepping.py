...
def rk_advance(state,kgrid,params):
    ...
    #RK4 substep 2
    k2 = NonlinearTerm(grad(SimulationState(state.t+dt/2.0,f1),kgrid),kgrid)
    f2 = jax.tree_util.tree_map(lambda fn, kn, dh : dh * fn + 0.5 * dt * kn, state.fields, k2,diss_half)
    #RK4 substep 3
    k3 = NonlinearTerm(grad(SimulationState(state.t+dt/2.0,f2),kgrid),kgrid)
    f3 = jax.tree_util.tree_map(lambda fn, kn, df, dh : df * fn + dt * dh * kn, state.fields, k3, diss_full, diss_half)
    #RK4 final step
    k4 = NonlinearTerm(grad(SimulationState(state.t+dt,f3),kgrid),kgrid)
