...
def estimate_good_nblock(state,kgrid,params,t_snap,t_end,t_last_snap=0,nblock_min=10):
    set_timestep,_,grad,_,_ = equation_registry[params.eqtype] 
...
def block_of_steps(state,kgrid,linmats,params,nblock,scheme,stepper):
    def stepping(state,_):
        ...
        return stepper(state,kgrid,params,rhs,linmats,set_timestep,scheme), None
...
def simulate_scan(state,kgrid,linmats,params,nblock,t_snap,t_end,mngr,schemestr='lsrk33',save=True):
    ...
    block_of_steps_jit = jax.jit(block_of_steps,static_argnums=(3,4,5,6))
    ...
    while state.t<t_end:
        state, _ = block_of_steps_jit(state,kgrid,linmats,params,nblock,scheme,stepper)
        if params.rank==0:
            print(state.t)
...
#the below is out of date; just use scan
